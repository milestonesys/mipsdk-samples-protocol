...
def main():
    username = "username"  # Replace with an XProtect user with the XProtect Administrators role
    password = "password"  # Replace with password for the user
    ...
    response = identity_provider.get_token(
        session, username, password, serverUrl, isBasicUser
    )
...
def crud_user_defined_event(
    api_gateway: Gateway, session: requests.Session, token: str
):
    ...
    response = api_gateway.update_item(
        session, "userDefinedEvents", payload, event_id, token
    )
    if response.status_code == 200:
...
def cameras_and_tasks(
    api_gateway: Gateway, session: requests.Session, token: str, camera_id: str
):
    """Find, invoke, and clean up a camera device task"""

    # Get a list of available tasks on camera PTZ presets
    response = api_gateway.get_child_item_tasks(
        session, "cameras", camera_id, "ptzpresets", token
    )
    if response.status_code == 200:
    ...
    if get_device_presets_task is None:
        print(
            f"Camera PTZ presets does not have a GetDevicePresets task, use another camera\n\n"
        )
        return
    ...
    response = api_gateway.perform_child_task(
        session, "cameras", camera_id, "ptzpresets", "GetDevicePresets", payload, token
    )
    if response.status_code == 200:
    ...
    response = api_gateway.perform_task(
        session, "tasks", task_id, "TaskCleanup", "{}", token
    )
    if response.status_code == 200:
        task_result = response.json()["result"]
